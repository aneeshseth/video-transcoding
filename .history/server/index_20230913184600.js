...
app.post("/api/posts",upload.single('video'), async (req,res) => {
    ...
    const uuidName = `${Date.now()}`;
    const converTo = ['100k', '800k', '1200k', '2400k', '3000k'];
    const m3u8Name = `${uuidName}.m3u8`;
    try {
        const ffmpegPromises = converTo.map( async (bitrate) => {
            const outputFileName = `${bitrate}-${uuidName}.m3u8`;
            try {
                ffmpeg()
                .input(url)
                .outputOptions([
